...
```{r}
# Dette virker bare på de som er listet blandt de 10 første.
# Det er mange mer. Bruk heller regular expressions (kap. 14 i r4ds)
# sett pattern = "\\s*\\([-\\d]+\\)\\s*"
# i ord: null eller flere white-space \\s*, opp til en parentes 
# start \\(, så bindestrek eller tall én eller flere 
# ganger [-\\d]+ opp til parentes slutt \\), så til slutt 
# null eller flere white-space \\s*
# vi må benytte \\ for escape i R
...
```{r}
head(unique(arblos_aldersgrp$knavn), n = 40)
```
...
Her lever dere farlig. Siden dere ikke angir `by` argumentet vil den matche på knr, knavn, aar og mnd. Dere er derfor avhengig av at navnet på kommunene er skrevet nøyaktig likt i filene dere merger. Dette kan veldig fort gå galt og feilen vil være vanskelig å finne. Jeg vil anbefale å matche på knr, aar og mnd. Dere vil da få to variabler knavn.x og knavn.y. Dere kan så sjekke om knavn.x er nøyaktig lik knavn.y. Er den det kan en av dem slettes og navnet på den gjenværende endres til knavn. Hvis ikke identisk lik bør dere velge hvilken variant av knavn dere ønsker, event. endre tidligere i dokumentet slik at navnene er like.

```{r}
# check that we have the same knavn
stopifnot(identical(df_arbl$knavn, df_arbl_p_kjonn$knavn))
stopifnot(identical(df_arbl$knavn, arblos_aldersgrp$knavn))
          
# arbeidsløse 1999 til 2014 månedlige data
# will match on knr, knavn, aar, mnd
al9914m <- df_arbl %>% 
